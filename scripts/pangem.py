...
def get_file_names(directory1, directory2):
    onlyfiles = [f for f in os.listdir(directory1) if os.path.isfile(os.path.join(directory1, f)) and f.endswith('.gbk')]
    gl8 = [os.path.splitext(f)[0] for f in onlyfiles]
    gl9 = pd.DataFrame({'strain': gl8, 'NCBI ID': gl8, 'Pathotype': range(len(onlyfiles))})
    path = os.path.join(directory2, 'StrainInformation.xlsx')
    gl9.to_excel(path, index=False)
...
def run_blastn(seq, db,outfmt=6,evalue=0.001,threads=96):
    import os
...
def parse_nucl_blast(infile):
    ...
    data = data[(data['PID']>80) & (data['alnLength']>0.3*data['queryEnd'])]
...
for strain in targetStrainIDs:
    gbk2fasta('target_genome_dir/'+str(strain)+'.gbk')
...
genome_blast_res=[]
for strain in targetStrainIDs:
    res = run_blastn(referenceStrainID,str(strain))
